...
const Home = ({projects}: Props) => {
  ...
  return (
    <div className="h-screen w-screen">
      {!isMobile &&
        <Spline className="absolute z-0" scene="https://prod.spline.design/byrEA1hu6JsTNFD8/scene.splinecode"/>}
      <div className="w-full h-full p-2 rounded-lg bg-color-corners">
        <div
          className="relative w-full h-full px-5 py-2 bg-stone-800 text-white font-silkscreen rounded-sm overflow-hidden">
          <TopBar/>
          ...
          <div className="mt-10">
            ...
            <div
              className="relative h-full mt-6 flex flex-col gap-2 text-sm text-slate-200 before:absolute before:-left-4 before:w-0.5 before:h-full before:bg-gradient-to-tr before:from-indigo-500 before:to-purple-500">
              {carouselIndex !== -1 ? (
                <p
                  className="overflow-y-scroll">{carouselIndex !== -1 && projects[carouselIndex].description}</p>
              ) : (
                <>
                  <p>
          ...
          <div className="absolute bottom-12 w-full flex items-start flex-col">
            <div className="w-full flex justify-center mb-3">
              <h2
                className="relative text-xl before:absolute before:-left-14 before:top-1/2 before:w-10 before:h-0.5 before:bg-purple-500 after:absolute after:-right-14 after:top-1/2 after:w-10 after:h-0.5 after:bg-purple-500">
                projects
              </h2>
            </div>
            <ul className="flex flex-col gap-3">
              {projects.map((project: ProjectFragment, index: number) => (
                <li onClick={() => setCarouselIndex(index)} key={project.id}>
                  {project.main && '(*) '}
                  {project.name}
                </li>
              ))}
